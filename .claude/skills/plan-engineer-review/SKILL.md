---
name: plan-engineer-review
description: |
  Review and lock an engineering execution plan before code changes. Evaluate
  architecture, data flow, diagrams, edge cases, test coverage, performance,
  security, failure modes, and implementation scope. Use this skill whenever a
  user asks for an architecture review, engineering review, plan review,
  implementation-plan check, or wants a design document challenged before coding.
when_to_use: |
  Use for requests such as "review the architecture", "engineering review",
  "lock in the plan", "tech review", "technical review", or "check the
  implementation plan". Also use when the user provides a plan or design doc
  and is about to start coding.
...
If the skill was invoked with `$target`, treat the argument as the candidate plan, design document, file, directory, or branch target. Confirm that target in the first AskUserQuestion before reading or inspecting it. If `$target` is empty, use the same choices below and wait for the user's selection.

## AskUserQuestion format

For each non-trivial issue, ask one question at a time. State the issue and its user impact, recommend one option with a concrete reason, and list the tradeoffs, effort, risk, and maintenance burden for each option. If options differ in coverage, include a completeness score for each option; if they differ in kind, say that no completeness score applies.

## Scope gate (FIRST — overrides everything below). This is a hard STOP.

Before ANYTHING else in this skill — before the Design Doc Check, Step 0, and any `git` / `Read` / `Grep` / `Glob` / `Bash` call — your VERY FIRST tool call MUST be AskUserQuestion, to confirm the review target. Do not run the Design Doc Check bash or explore the repo before the user answers.
...
|------|-------------------|
| running the 4-section review, required outputs, and review report (only after Step 0 scope is agreed) | `${CLAUDE_SKILL_DIR}/sections/review-sections.md` |
---
...
DESIGN=$(find "$PROJECT_DIR" -maxdepth 3 -type f \( -iname "*-$BRANCH-design-*.md" -o -iname "*-design-*.md" \) -print 2>/dev/null | sort | tail -1)
...
> **STOP.** Before running the 4-section review, required outputs, and review report (only after Step 0 scope is agreed), Read `${CLAUDE_SKILL_DIR}/sections/review-sections.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

## Section self-check (before you finish)

Confirm you Read the review section the Section index named, and executed every review section (Architecture, Code Quality, Tests, Performance) and the required outputs in full. If you produced findings or the review report from memory without Reading `${CLAUDE_SKILL_DIR}/sections/review-sections.md`, stop and Read it now.
...
2. Confirm the LAST `## ` heading in the file is `## ENGINEERING REVIEW REPORT`.
   In-body prose that mentions review findings or similar does NOT count — only
   the structured `## ENGINEERING REVIEW REPORT` section satisfies this check.
3. Confirm the report has a Runs / Status / Findings table and a VERDICT line.
4. Confirm the report's FINAL non-whitespace line is the unresolved-decisions
   status: the exact unbolded `NO UNRESOLVED DECISIONS`, or a bullet of a final
   `**UNRESOLVED DECISIONS:**` block. BLOCKING, no "if applicable" escape — a
   bolded sentinel, any trailing prose, or a missing status each FAILS the gate.
