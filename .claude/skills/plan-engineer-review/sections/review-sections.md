...
framework-aware review design doc.
...
Follow the AskUserQuestion format described in `${CLAUDE_SKILL_DIR}/SKILL.md`. Additional rules for plan reviews:
...
Emit the markdown section below.
...
After completing the review, update the plan file itself so the review status is visible to anyone reading the plan.

### Detect the plan file

1. Check if there is an active plan file in this conversation. The host may provide a plan file path in the system context.
2. If no plan file is available, skip this section silently — not every review runs in plan mode.

### Generate the report

Read the review findings and completion summary from this review. Produce this markdown table at the end of the plan file:
...
|--------|---------|-----|------|--------|----------|
| Engineering Review | `/plan-engineer-review` | Architecture, code quality, tests, and performance | 1 | {status} | {issues_found} issues, {critical_gaps} critical gaps |
```

Below the table, add:

- **VERDICT:** state whether the plan is ready to implement and identify any required follow-up.
- **UNRESOLVED DECISIONS:** list each decision the user did not answer. If there are none, end with the exact unbolded line `NO UNRESOLVED DECISIONS`.

### Write to the plan file

The report must always be the LAST section of the plan file — never mid-file.

1. Read the plan file with the Read tool to see its full current content. Search for an existing `## ENGINEERING REVIEW REPORT` heading.
2. If found, use the Edit tool to delete the entire existing section through the next `## ` heading or end of file.
3. Append the new `## ENGINEERING REVIEW REPORT` section at the end of the file.
4. Verify with the Read tool that `## ENGINEERING REVIEW REPORT` is the last `## ` heading in the file before continuing.

Do not replace the report in place when an older report exists in the middle of the file. Delete the old section first, then append the new report at the end.
